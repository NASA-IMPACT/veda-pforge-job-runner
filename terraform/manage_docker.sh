...
IMAGE_NAME="beam-pyspark-emr-serverless"
...
if [ "$1" = "test" ]; then
    ...
    validate_docker_image "$IMAGE_NAME" "emr-7.0.0"
...
# Push process
    ...
    aws ecr get-login-password --region us-west-2 | docker login --username AWS --password-stdin "${REPO_URL%%/${REPO_NAME}*}"
    if [ $? -ne 0 ]; then
